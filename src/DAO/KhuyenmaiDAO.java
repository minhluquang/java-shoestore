...
public class KhuyenmaiDAO {
	...
	public static ArrayList<KhuyenMai> searchKhuyenMai(String keyword, int status) {
		...
		ArrayList<KhuyenMai> dskm = new ArrayList<>();
		try {
			String sql = "SELECT * FROM `discounts` WHERE (discount_code LIKE '%" + keyword + "%' OR status LIKE '%"
					+ keyword + "%' OR discount_value LIKE '%" + keyword + "%' OR start_date LIKE '%" + keyword
	...
	public static boolean insertKhuyenMai(ArrayList<KhuyenMai> dskm) {
		...
		for (KhuyenMai nv : dskm) {
			String name = nv.getDiscount_code();
			int value = nv.getDiscount_value();
			String type = nv.getType();
			String start = nv.getStart_date();
			String end = nv.getEnd_date();
			int status = 1;

			System.out.println(name);

			success = insertKhuyenMai(name, value, type, start, end, status);
			if (!success) {
				return success;
			}
		}
